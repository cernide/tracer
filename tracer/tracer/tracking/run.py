...
from tracer.events import LoggedEventSpec, V1Event, V1EventSpan, get_asset_path
from tracer.logger import logger
from tracer.logging import V1Log, V1Logs
from tracer.processors import events_processors
from tracer.processors.logs_processor import end_log_processor, start_log_processor
...
class Run(RunClient):
    ...
    def __init__(
        ...
        has_process_sidecar = os.environ.get(
            ENV_KEYS_HAS_PROCESS_SIDECAR, False)
    ...
    def _init_artifacts_tracking(
        ...
        if not self._artifacts_path:
            artifacts_path = ctx_paths.CONTEXT_ARTIFACTS_FORMAT.format(
                self.run_uuid)
            self.set_artifacts_path(artifacts_path)
    ...
    def _add_event(self, event: LoggedEventSpec):
        if self._event_logger:
            ...
            logger.warning(
                "Could not log event {}, "
                "the event logger was not configured properly".format(
                    event.name)
            )
        ...
        if self._logs_history.logs and len(self._logs_history.logs) > 0:
            logs_path = os.path.join(
                ...
                "{}".format(datetime.timestamp(
                    self._logs_history.logs[-1].timestamp)),
            )
    ...
    def set_run_process_sidecar(self):
        ...
        > to automatically sync your run's artifacts and outputs.
        """
        self._sidecar = SidecarThread(
            client=self, run_path=self._artifacts_path)
        self._sidecar.start()
    ...
    def log_sklearn_roc_auc_curve(
        ...
        if is_multi_class:
            ...
            for i in range(len(classes)):
                create_event(
                    "{}_{}".format(
                        name, i), y_preds[:, i], y_targets, classes[i]
                )
        else:
    ...
    def log_confusion_matrix(
        ...
        logged_event = LoggedEventSpec(
            ...
            event=V1Event.make(timestamp=timestamp,
                               step=step, confusion=event_value),
        )
        self._add_event(logged_event)
    ...
    def log_mpl_image(
        ...
        if isinstance(data, list):
            event_value = events_processors.figures_to_images(
                figures=data, close=close)
            ...
            event_value = events_processors.figure_to_image(
                figure=data, close=close)
            self.log_image(
                name=name,
                data=event_value,
                step=step,
    ...
    def log_np_histogram(
        ...
        event_value = events_processors.np_histogram(
        ...
        logged_event = LoggedEventSpec(
            ...
            event=V1Event.make(timestamp=timestamp,
                               step=step, histogram=event_value),
        )
        self._add_event(logged_event)
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            if content_type == V1ArtifactKind.CSV:
                ...
                raise ValueError(
                    ...
                    "This method supports `csv` or `html`.".format(
                        content_type)
                )
    ...
    def log_mpl_plotly_chart(
        ...
        def log_figure():
            ...
            chart = events_processors.mpl_plotly_chart(
                figure=figure, close=close)
            logged_event = LoggedEventSpec(
                name=self._sanitize_filename(name),
                kind=V1ArtifactKind.CHART,
                event=V1Event.make(timestamp=timestamp,
                                   step=step, chart=chart),
            )
            self._add_event(logged_event)
        ...
        try:
            ...
            if fallback_to_image:
                logger.warning(
                    "Could not convert figure to plotly. Error %s", e)
                self.log_mpl_image(
    ...
    def _start(self):
        if self._is_offline:
            ...
            if self.run_data.status:
                logger.info(
                    f"An offline run was found: {self._artifacts_path}")
            else:
                self.log_status(
                    V1Statuses.CREATED,
                ...
                logger.info(
                    f"A new offline run started: {self._artifacts_path}")
    ...
    def log_env(self, rel_path: Optional[str] = None, content: Optional[Dict] = None):
        ...
        if not os.path.exists(self._outputs_path):
            return
        if not content:
            content = get_run_env(["polyaxon", "tracer"])
