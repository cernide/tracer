...
from tracer.events.schemas import (
    LoggedEventListSpec,
    V1Event,
    V1EventArtifact,
    V1EventAudio,
...
class TestBaseEvent(BaseTestCase):
    ...
    def test_log_line_has_datetime(self):
        parsed = V1Event.make(
            timestamp="2018-12-11 10:24:57", metric=2.2, step=12)
        expected = V1Event(
...
class TestEventsV1(BaseTestCase):
    ...
    def test_metrics_read_yaml(self):
        ...
        events = V1Events.read(
            ...
            data=os.path.abspath(
                "tests/fixtures/events/metric/metric_events.plx"),
        )
            ...
            events=[
                V1Event(
                    ...
                    image=V1EventImage(height=1, width=1,
                                       colorspace=1, path="path"),
                    step=12,
                ),
                V1Event(
                    timestamp=parse_datetime("2018-12-11 11:24:57"),
                    image=V1EventImage(height=10, width=1,
                                       colorspace=0, path="path"),
                    step=13,
                ),
                V1Event(
                    timestamp=parse_datetime("2018-12-11 12:24:57"),
                    image=V1EventImage(height=1, width=10,
                                       colorspace=2, path="path"),
                    step=14,
                ),
            ],
        )
    ...
    def test_images_read_yaml(self):
        ...
        events = V1Events.read(
            ...
            kind="image",
            data=os.path.abspath(
                "tests/fixtures/events/image/image_events.plx"),
        )
    ...
    def test_video(self):
        events = LoggedEventListSpec(
            ...
            events=[
                ...
                V1Event(
                    timestamp=parse_datetime("2018-12-11 11:24:57"),
                    video=V1EventVideo(height=10, width=1,
                                       colorspace=0, path="path"),
                    step=13,
                ),
                V1Event(
                    timestamp=parse_datetime("2018-12-11 12:24:57"),
                    video=V1EventVideo(height=1, width=10,
                                       colorspace=2, path="path"),
                    step=14,
                ),
            ],
    ...
    def test_videos_read_yaml(self):
        ...
        events = V1Events.read(
            ...
            kind="video",
            data=os.path.abspath(
                "tests/fixtures/events/video/video_events.plx"),
        )
        assert events.name == "foo"
        assert len(events.df.values) == 3
        for i in range(3):
    ...
    def test_audios_read_yaml(self):
        ...
        events = V1Events.read(
            ...
            kind="audio",
            data=os.path.abspath(
                "tests/fixtures/events/audio/audio_events.plx"),
        )
        assert events.name == "foo"
        assert len(events.df.values) == 3
        for i in range(3):
    ...
    def test_charts_read_yaml(self):
        ...
        events = V1Events.read(
            ...
            kind="chart",
            data=os.path.abspath(
                "tests/fixtures/events/chart/chart_events.plx"),
        )
        assert events.name == "foo"
        assert len(events.df.values) == 3
        for i in range(3):
    ...
    def test_curves_read_yaml(self):
        ...
        events = V1Events.read(
            ...
            kind="curve",
            data=os.path.abspath(
                "tests/fixtures/events/curve/curve_events.plx"),
        )
        assert events.name == "foo"
        assert len(events.df.values) == 3
        for i in range(3):
    ...
    def test_models_read_yaml(self):
        ...
        events = V1Events.read(
            ...
            kind="model",
            data=os.path.abspath(
                "tests/fixtures/events/model/model_events.plx"),
        )
        assert events.name == "foo"
        assert len(events.df.values) == 3
        for i in range(3):
            ...
            events=[
                V1Event(
                    ...
                    dataframe=V1EventDataframe(
                        path="path", content_type="parquet"),
                    step=12,
                ),
                V1Event(
                    timestamp=parse_datetime("2018-12-11 11:24:57"),
                    dataframe=V1EventDataframe(
                        path="path", content_type="pickle"),
                    step=13,
                ),
                V1Event(
                    timestamp=parse_datetime("2018-12-11 12:24:57"),
    ...
    def test_dataframes_read_yaml(self):
        values = [
            V1Event(
                timestamp=parse_datetime("2018-12-11 10:24:57"),
                dataframe=V1EventDataframe(
                    path="path1", content_type="parquet"),
                step=12,
            ),
            V1Event(
                timestamp=parse_datetime("2018-12-11 10:25:57"),
                dataframe=V1EventDataframe(
                    path="path2", content_type="pickle"),
                step=13,
            ),
            V1Event(
                timestamp=parse_datetime("2018-12-11 10:26:57"),
    ...
    def test_spans_read_yaml(self):
        ...
        values = [
            V1Event(
                timestamp=parse_datetime("2018-12-11 10:24:57"),
                span=V1EventSpan(uuid=uuid_hex, name="span1",
                                 tags=["tag1", "tag2"]),
                step=12,
            ),
            V1Event(
            ...
            V1Event(
                timestamp=parse_datetime("2018-12-11 10:26:57"),
                span=V1EventSpan(uuid=uuid_hex, name="span3",
                                 tags=["tag1", "tag2"]),
                step=14,
            ),
        ]
