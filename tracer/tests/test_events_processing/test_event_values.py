...
from tracer.processors.events_processors import (
    audio,
    bokeh_chart,
    convert_to_HWC,
    histogram,
...
class TestEventValues(BaseTestCase):
    ...
    def test_video(self):
        ...
        event = video(asset_path=asset_path,
                      tensor=tensor_np(shape=(4, 3, 1, 8, 8)))
        ...
        assert os.path.exists(asset_path) is True
        event = video(asset_path=asset_path,
                      tensor=tensor_np(shape=(20, 7, 1, 8, 8)))
        assert event.path == asset_path
        assert os.path.exists(asset_path) is True
    ...
    def test_to_HWC(self):  # noqa
        ...
        test_image = np.random.randint(
            0, 256, size=(3, 32, 32), dtype=np.uint8)
        converted = convert_to_HWC(test_image, "chw")
        assert converted.shape == (32, 32, 3)
        test_image = np.random.randint(
            0, 256, size=(16, 3, 32, 32), dtype=np.uint8)
