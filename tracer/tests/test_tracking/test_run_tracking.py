...
from tracer.events import V1Events, get_asset_path, get_event_path
from tracer.serialization.writer import EventFileWriter, ResourceFileWriter
...
class TestRunTracking(TestEnvVarsCase):
    ...
    def test_run_init(self, expanduser):
        ...
        with patch("tracer.tracking.run.Run._set_exit_handler") as exit_mock:
        ...
        # FQN non CE
        settings.CLI_CONFIG.installation = {"dist": _dist.EE}
        os.environ[ENV_KEYS_RUN_INSTANCE] = "user.project_bar.runs.{}".format(
            uid)
        run = Run()
    ...
    def test_event_logger_from_non_managed_run(self):
        ...
        assert exit_mock.call_count == 1
        artifacts_context = ctx_paths.CONTEXT_ARTIFACTS_FORMAT.format(
            run.run_uuid)
        ...
        with patch("tracer.tracking.run.EventFileWriter") as mock_call:
            run.set_run_event_logger()
        assert mock_call.call_count == 1

        with patch("tracer.tracking.run.ResourceFileWriter") as mock_call:
        ...
        settings.CLIENT_CONFIG.is_managed = True
        with patch("tracer.tracking.run.EventFileWriter") as event_call:
            with patch("tracer.tracking.run.ResourceFileWriter") as resource_call:
                with patch("tracer.tracking.run.Run.refresh_data") as refresh_call:
                    with patch(
                        "tracer.tracking.run.Run._set_exit_handler"
                    ) as exit_call:
    ...
    def test_event_logger_from_a_managed_run(self):
        ...
        # Add run id
        uid2 = uuid.uuid4().hex
        with patch("tracer.tracking.run.Run.set_run_event_logger") as event_call:
            with patch(
                "tracer.tracking.run.Run.set_run_resource_logger"
            ) as resource_call:
                with patch("tracer.tracking.run.Run.refresh_data") as refresh_call:
                    with patch(
                        "tracer.tracking.run.Run._set_exit_handler"
...
class TestRunLogging(TestEnvVarsCase):
    ...
    def test_log_empty_metric(self):
        assert (
            os.path.exists(get_event_path(
                self.run_path, kind=V1ArtifactKind.METRIC))
            is False
        )
        assert (
            os.path.exists(get_asset_path(
                self.run_path, kind=V1ArtifactKind.METRIC))
            is False
        )
        with patch("tracer.tracking.run.Run._log_has_metrics") as log_metrics:
    ...
    def test_log_single_metric(self):
        ...
        results = V1Events.read(
            kind="metric", name="metric1", data=events_file)
    ...
    def test_log_multiple_metrics(self):
        ...
        assert os.path.exists(events_file) is True
        results = V1Events.read(
            kind="metric", name="metric2", data=events_file)
        assert len(results.df.values) == 1

        with patch("tracer.tracking.run.Run._log_has_metrics") as log_metrics:
            self.run.log_metrics(step=2, metric1=1.1,
                                 metric2=21.1, metric3=12.1)
        assert log_metrics.call_count == 1
        self.event_logger.flush()
        assert (
            os.path.exists(get_asset_path(
        ...
        assert os.path.exists(events_file) is True
        results = V1Events.read(
            kind="metric", name="metric3", data=events_file)
        assert len(results.df.values) == 1

    def test_log_image_from_path(self):
        assert (
            os.path.exists(get_event_path(
                self.run_path, kind=V1ArtifactKind.IMAGE))
            is False
        )
        assert (
            os.path.exists(get_asset_path(
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_image:
        ...
        assert os.path.exists(events_file) is True
        results = V1Events.read(
            kind="image", name="my_image", data=events_file)
        assert len(results.df.values) == 1
    ...
    def test_log_image_with_boxes(self):
        ...
        image_file = tempfile.mkdtemp() + "/file.png"
        self.touch(image_file)
        with patch("tracer.tracking.run.Run._log_has_events") as log_image_with_boxes:
    ...
    def test_log_mpl_image(self):
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_mpl_image:
            self.run.log_mpl_image(
                name="figure", data=figure, step=1, close=False)
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_dashboard:
    ...
    def test_log_mpl_images(self):
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_mpl_image:
            self.run.log_mpl_image(
                name="figure", data=figures, step=1, close=False)
        assert log_mpl_image.call_count == 1
        assert all([plt.fignum_exists(figure.number)
                   is True for figure in figures])
        ...
        assert log_mpl_image.call_count == 1
        assert all([plt.fignum_exists(figure.number)
                   is False for figure in figures])
    ...
    def test_log_mpl_plotly(self):
        assert (
            os.path.exists(get_event_path(
                self.run_path, kind=V1ArtifactKind.CHART))
            is False
        )
        assert (
            os.path.exists(get_asset_path(
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_mpl_plotly_chart:
    ...
    def test_log_video_from_path(self):
        assert (
            os.path.exists(get_event_path(
                self.run_path, kind=V1ArtifactKind.VIDEO))
            is False
        )
        assert (
            os.path.exists(get_asset_path(
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_video:
        ...
        assert os.path.exists(events_file) is True
        results = V1Events.read(
            kind="video", name="my_video", data=events_file)
        assert len(results.df.values) == 1
    ...
    def test_log_data_video(self):
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_dashboard:
            self.run.log_video(
                name="my_video", data=tensor_np(shape=(4, 3, 1, 8, 8)))
        assert log_dashboard.call_count == 1
        self.event_logger.flush()
        assert (
            os.path.exists(get_asset_path(
    ...
    def test_log_audio_from_path(self):
        assert (
            os.path.exists(get_event_path(
                self.run_path, kind=V1ArtifactKind.AUDIO))
            is False
        )
        assert (
            os.path.exists(get_asset_path(
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_audio:
        ...
        assert os.path.exists(events_file) is True
        results = V1Events.read(
            kind="audio", name="my_audio", data=events_file)
        assert len(results.df.values) == 1
    ...
    def test_log_text(self):
        assert (
            os.path.exists(get_event_path(
                self.run_path, kind=V1ArtifactKind.TEXT))
            is False
        )
        assert (
            os.path.exists(get_asset_path(
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_text:
    ...
    def test_log_html(self):
        assert (
            os.path.exists(get_event_path(
                self.run_path, kind=V1ArtifactKind.HTML))
            is False
        )
        assert (
            os.path.exists(get_asset_path(
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_html:
    ...
    def test_log_histogram(self):
        assert (
            os.path.exists(get_event_path(
                self.run_path, kind=V1ArtifactKind.HISTOGRAM))
            is False
        )
        assert (
            os.path.exists(get_asset_path(
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_histogram:
        ...
        assert os.path.exists(events_file) is True
        results = V1Events.read(
            kind="histogram", name="histo", data=events_file)
    ...
    def test_log_np_histogram(self):
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_np_histogram:
    ...
    def test_log_model_file(self):
        assert (
            os.path.exists(get_event_path(
                self.run_path, kind=V1ArtifactKind.MODEL))
            is False
        )
        assert (
            os.path.exists(get_asset_path(
        ...
        assert (
            os.path.exists(self.run.get_outputs_path(
                V1ArtifactKind.MODEL.value))
            is False
        )
        model_file = tempfile.mkdtemp() + "/model.pkl"
        self.touch(model_file)
        with patch("tracer.tracking.run.Run.log_model_ref") as log_model:
            self.run.log_model(
                name="my_model",
                path=model_file,
                framework="scikit",
    ...
    def test_log_versioned_model_file(self):
        ...
        model_file = tempfile.mkdtemp() + "/model.pkl"
        self.touch(model_file)
        with patch("tracer.tracking.run.Run._log_has_model") as log_model:
            self.run.log_model(
        ...
        assert os.path.exists(events_file) is True
        results = V1Events.read(
            kind="model", name="my_model", data=events_file)
        assert len(results.df.values) == 1
    ...
    def test_log_dataframe_ref(self):
        assert (
            os.path.exists(get_event_path(
                self.run_path, kind=V1ArtifactKind.DATAFRAME))
            is False
        )
        assert (
            os.path.exists(get_asset_path(
        ...
        self.touch(model_file)
        with patch("tracer.tracking.run.Run.log_artifact_ref") as log_artifact_ref:
    ...
    def test_log_dataframe(self):
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_dataframe:
        ...
        assert os.path.exists(events_file) is True
        results = V1Events.read(
            kind="dataframe", name="dataframe", data=events_file)
        assert len(results.df.values) == 1
    ...
    def test_log_artifact(self):
        assert (
            os.path.exists(get_event_path(
                self.run_path, kind=V1ArtifactKind.TSV))
            is False
        )
        assert (
            os.path.exists(get_asset_path(
        ...
        self.touch(tsv_file)
        with patch("tracer.tracking.run.Run.log_artifact_ref") as log_artifact:
            self.run.log_artifact(
                name="file",
                path=tsv_file,
    ...
    def test_versioned_log_artifact(self):
        ...
        tsv_file = tempfile.mkdtemp() + "/file.tsv"
        self.touch(tsv_file)
        with patch("tracer.tracking.run.Run._log_has_events") as log_artifact:
            self.run.log_artifact(
    ...
    def test_log_artifact_without_name(self):
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_artifact:
            self.run.log_artifact(
                path=tsv_file, kind=V1ArtifactKind.TSV, step=1)
        assert log_artifact.call_count == 1
        self.event_logger.flush()
        assert (
            os.path.exists(get_asset_path(
    ...
    def test_log_artifact_without_name_and_filename_with_several_dots(self):
        assert (
            os.path.exists(get_event_path(
                self.run_path, kind=V1ArtifactKind.FILE))
            is False
        )
        assert (
            os.path.exists(get_asset_path(
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_artifact:
            self.run.log_artifact(
                path=tar_file, kind=V1ArtifactKind.FILE, step=1)
        assert log_artifact.call_count == 1
        self.event_logger.flush()
        assert (
            os.path.exists(get_asset_path(
        ...
        assert os.path.exists(events_file) is True
        results = V1Events.read(kind=V1ArtifactKind.FILE,
                                name="file", data=events_file)
        assert len(results.df.values) == 1
    ...
    def test_log_charts(self):
        ...
        df1 = pd.DataFrame(
            [["A", "B", "C", "D"], [28, 55, 43, 91]], index=["a", "b"]).T
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_charts:
            self.run.log_bokeh_chart(
                name="bokeh_test", figure=bokeh_test, step=1)
            self.run.log_plotly_chart(
                name="plotly_test", figure=plotly_test, step=1)
    ...
    def test_log_curves(self):
        ...
        with patch("tracer.tracking.run.Run._log_has_events") as log_curves:
            self.run.log_roc_auc_curve(
                name="roc_test", fpr=x, tpr=y, auc=0.6, step=1)
            self.run.log_pr_curve(
                name="pr_test", precision=x, recall=y, average_precision=0.6, step=1
            )
            self.run.log_curve(name="curve_test", x=x,
                               y=y, annotation=0.6, step=1)
        assert log_curves.call_count == 3
        self.event_logger.flush()
        assert (
            os.path.exists(get_asset_path(
                self.run_path, kind=V1ArtifactKind.CURVE))
            is False
        )
        assert (
            os.path.exists(get_event_path(
