...
from tracer.processors import df_processors
...
class DataFrameSummaryTest(TestCase):
        ...
        self.df = pd.DataFrame(
            dict(
                ...
                ddates=pd.date_range(
                    "2010-01-01", periods=self.size, freq="1M"),
            )
        )
        self.column_stats = df_processors.get_df_column_stats(self.df)
        self.columns_types = df_processors.get_df_columns_types(
            self.column_stats)

    def test_get_columns_works_as_expected(self):
        assert (
            len(df_processors.get_df_columns(
                self.df, df_processors.DFUsage.ALL)) == 10
        )
    ...
    def test_column_stats_works_as_expected(self):
        ...
        # missing
        expected = pd.Series(index=self.columns, data=0,
    ...
    def test_categorical_summary(self):
        expected = pd.Series(
            index=["top", "counts", "uniques",
                   "missing", "missing_perc", "types"],
            data=["a: 500", self.size, 3, 0, "0%",
                  df_processors.DFTypes.CATEGORICAL],
            name="dcategoricals",
            dtype=object,
        )
        ...
        assert_series_equal(
            df_processors.get_df_column_summary(
                self.df, "dcategoricals"), expected
        )
    ...
    def test_dates_summary(self):
        ...
        tmp = df_processors.get_df_column_summary(
            self.df, "ddates").sort_index()
