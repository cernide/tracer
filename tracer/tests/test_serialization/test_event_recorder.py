...
from tracer.serialization.writer import EventAsyncManager, EventFileWriter, EventWriter
...
class TestEventWriter(BaseTestCase):
    ...
    def test_init_events(self):
        ...
        expected_events = V1Events.read(
            kind="metric", name="test", data=event_file)
        ...
        expected_events = V1Events.read(
            kind="text", name="new", data=new_event_file)
        assert expected_events.name == events.name
        assert expected_events.kind == events.kind
    ...
    def test_add_event_create_files_and_batch_events(self):
        ...
        ew.write(events)
        for event in events:
            new_event_file = ew._get_event_path(
                kind=event.kind, name=event.name)
            assert os.path.exists(new_event_file) is True
